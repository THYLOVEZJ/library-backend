...
public class JwtInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        // 执行认证
        if (StrUtil.isBlank(token)) {
            throw new MyException(ERROR_CODE_401, "无token，请重新登录");
        }
        // 获取 token 中的adminId
        String adminId;
        Admin admin;
        try {
            ...
            throw new MyException(ERROR_CODE_401, errMsg);
        }
        if (admin == null) {
            throw new MyException(ERROR_CODE_401, "用户不存在，请重新登录");
        }
        ...
        try {
            ...
            throw new MyException(ERROR_CODE_401, "token验证失败，请重新登录");
        }
        return true;
    }
}
